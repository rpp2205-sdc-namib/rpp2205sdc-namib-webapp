...
class RPList extends React.Component {
  ...
  render () {
    {console.log('here in RPList', this.props.relatedProds)};
    return (
      <div id="rpList">
        ...
        {this.state.rp.map((element, index) => {
          return(
            <RPC action={true} info={element} show={this.handleClick.bind(this)} key={index} redirect={this.props.changeProduct}/>
          )
         })
        }
        <button>Next</button>
